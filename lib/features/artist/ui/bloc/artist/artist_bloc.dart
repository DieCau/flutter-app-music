...
class ArtistBloc extends Bloc<ArtistEvent, ArtistState> {
  ...
  Future<void> fetchListMusicArtist(FetchListMusicArtist event, Emitter<ArtistState> emit) async {
    emit(state.copyWith(status: ArtistStatus.loading));
    try {
      final response = await artistRepository.fetchAllTracksArtist(url: event.urlPath);
  ...
  Future<void> fetchArtist(FetchArtist event, Emitter<ArtistState> emit) async {
    ...
    try {
      ...
      emit(state.copyWith(
          currentArtistStatus: CurrentArtistStatus.success, currentArtist: response));
    } catch (e) {
